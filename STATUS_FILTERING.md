...
### Channel Mentions
When you use `@SpinBot` directly in a channel:
1. Bot fetches all channel members
2. Checks each member's Slack status
3. Excludes members whose status contains configured emojis
4. Selects randomly from remaining members
...
**This is by design!**
- Thread mode: Selects from participants (no status check, no limits)
- Channel mode: Checks status for all members

To use status filtering, mention the bot directly in a channel.
...
### Performance
- All users are fetched once at the start
- Status information is included in the user data
- Very fast - no individual API calls per user
- Works efficiently even with large channels

### Rate Limits
Slack API limits:
- Users are fetched once per request
- Efficient Map-based lookups
- No risk of hitting rate limits
- Bot includes error handling for all API calls
...
A: Yes! The bot efficiently fetches all users once and uses Map lookups for status checks. No member limits.
